<script lang="ts">
	...
	import { getAllUsers } from '$lib/api/getAll';
	...
	function formatDateToBuddhistCalendar(dateString: string): string {
		...
		const fetchData = async () => {
			...
			if (token) {
				try {
					...
					users = userData.result.map((user: User) => ({
						id: user.id,
	...
	<div class="rounded-lg">
		...
		{#each filteredUsers as user}
			<div class="flex items-center px-4 py-3 hover:bg-gray-50 shadow-md my-2 relative">
				...
				<div class="flex-1 font-medium text-gray-900">{user.first_name}</div>
				<div class="w-28 text-gray-600">{user.id}</div>
				<div class="w-40 text-gray-600">
					สร้างเมื่อ {formatDateToBuddhistCalendar(user.created_at)}
				</div>
				<div class="w-48">
