...
<aside class="fixed left-0 top-0 z-50 flex h-screen w-[250px] flex-col justify-between bg-sucu-pink-05">
  <div>
    ...
    <nav class="flex flex-col p-4 gap-2">
      {#each menuItems as item}
        <a href={item.href} class="p-2 hover:bg-sucu-pink-04 font-semibold {currentPath === item.href ? 'bg-white' : ''}">
          {item.title}
        </a>
      {/each}
    </nav>
  ...
  <div>
    <div class="relative p-4 m-4 bg-white">
        <div class="absolute top-2 right-2">
            <button type="button" class="h-4 w-4 cursor-pointer" on:click={handleClick} aria-label="Edit">
    ...
    <div class="p-4 m-4 body-medium font-semibold hover:bg-sucu-pink-04">
        ออกจากระบบ
    </div>
